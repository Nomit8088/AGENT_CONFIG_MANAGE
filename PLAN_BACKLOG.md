...
| WI-013 | 待开发 | P1 | 同步交互重设计（方向化命名 + 逐插件对齐） | 同步按钮按数据流方向重组（上传到仓库 / 从仓库应用 / 预览差异），对齐从「整份 profile 全量覆盖」细化为「逐插件勾选决策」 | 按钮方向与后果明确；覆盖操作强制前置差异预览；diff 逐条勾选方向并逐条应用 | `SyncView.vue`、`DshPluginSync.vue`、`DshPluginDiffModal.vue`、`dshPlugins.ts`、`dsh_plugins_sync.rs` | ⭐ 下一步；吸收 WI-005/WI-008；WI-006 快照继续兜底 |
...
- **依赖**：`SyncView.vue`、`DshPluginSync.vue`、`DshPluginDiffModal.vue`、`dshPlugins.ts`（`alignDshPlugins`/`reconcileDshPlugins`）、`dsh_plugins_sync.rs`（Rust 双端对齐）、`DshPluginDiff`/`DshPluginEntry` 类型扩展。
- **风险/边界**：
...
| 7 | T1 | WI-013 | 同步交互重设计 | P1 | ⭐ 下一步；吸收 WI-005 / WI-008 的勾选与冲突解决 |
