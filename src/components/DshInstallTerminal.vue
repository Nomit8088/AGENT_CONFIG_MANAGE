  ...
  <div class="rounded-xl bg-[#121316] border border-black/8 dark:border-white/8 overflow-hidden transition-colors duration-200">
    ...
    >
      <span :class="store.dshInstallReport.ok ? 'text-[#22c55e]' : 'text-[#ef4444]'">
        {{ store.dshInstallReport.ok ? $t('dshPlugin.terminalInstallSuccess') : $t('dshPlugin.terminalInstallFail') }}
      </span>
...
const statusText = computed(() => {
  if (store.installTerminal.running) return t('dshPlugin.terminalRunning');
  if (store.dshInstallReport) {
    return store.dshInstallReport.ok ? t('dshPlugin.terminalDoneSuccess') : t('dshPlugin.terminalDoneFail');
  }
  return t('dshPlugin.terminalIdleStatus');
});
